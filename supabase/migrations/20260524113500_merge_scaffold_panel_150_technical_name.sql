...
begin
  ...
  where slug = 'painel-de-andaime-1-00-x-1-50-m'
  limit 1;

  select id into technical_id
  from public.equipments
  where slug = 'andaime-tubular-1-5-m-x-1-5-m'
  limit 1;

  if commercial_id is not null then
    update public.equipments
    set technical_name = 'Andaime tubular - 1,5 m x 1,5 m'
  ...
  if commercial_id is not null and technical_id is not null then
    ...
    set
      ...
      equipment_name = 'Painel de Andaime 1,00 × 1,50 m'
    where equipment_id = technical_id;
